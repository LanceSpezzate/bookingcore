    ...
    */
    ...
    public function socialCallBack($provider)
    {
        ...
        if(empty($existUser)){
            ...
            $realUser = User::query()->where('email',$user->getEmail())->first();
            if(!empty($realUser->id)){
                if($realUser->getMeta('social_'.$provider.'_id') == $user->getId()){

                    if(in_array($realUser->status,['blocked'])){
                        return redirect()->route('/login')->with('error',__('Your account has been blocked'));
                    }
                    ...
                    return redirect()->route('/login')->with('error',__('Email :email exists. Please select another one',['email'=>$user->getEmail()]));
                }
            }

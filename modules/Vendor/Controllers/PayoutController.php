...
class PayoutController extends FrontendController
{
    ...
    public function createPayoutRequest(){
        ...
        if(!is_array($vendor_payout_methods) or empty($vendor_payout_methods)){
            $this->sendError(__("Sorry! No method available at the moment"));
        }
        ...
        if(empty($user_available_methods) or empty($user_available_methods[$payout_method])){
            $this->sendError(__("You does not select payout method or you need to enter account info for that method"));
        }

        if($user->available_payout_amount < $amount){
            $this->sendError(__("You don not have enough :amount for payout",['amount'=>format_money($amount)]));
        }

        $method_detail = $user_available_methods[$payout_method];

        if(!empty($method_detail->min) and $method_detail->min > $amount){
            $this->sendError(__("Minimum amount to pay is :amount",["amount"=>format_money($method_detail->min)]));
        }
        ...
        if($payout->save())
        {
            ...
            $this->sendSuccess([],__("Payout request has been created"));

        }else{
            $this->sendSuccess([],__("Can not create vendor message"));
        }

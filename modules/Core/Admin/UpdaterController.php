...
class UpdaterController extends  AdminController
{
    ...
    public function doUpdate(){
        ...
        if(empty($updater_latest_version) or version_compare(config('app.version'),$updater_latest_version,'>=')){
            $this->sendError(__("You are using latest version of Booking Core"));
        }

        if(!class_exists('\ZipArchive'))
        {
            $this->sendError("Your server does not support ZipArchive libraries. Please contact your hosting to install it or do update manually");
        }
        ...
        {
            $this->sendError("The root folder is not able to write");
        }
        ...
        if(!$data or empty($data_json))
        {
            $this->sendError("Can not connect to update server. Please check again");
        }

        if(empty($data_json['file']))
        {
            $this->sendError(__("Can not get update file from server"));
        }
        ...
        try {
            ...
            $this->sendError("downloadFile: ".$exception->getMessage());
        }

        if(!file_exists($zip_file_tmp))
        {
            $this->sendError(__("Can not download update file to folder storage"));
        }
        ...
        if($check){
            ...
            $this->sendError(__("Can not un-zip the package"));
        }
